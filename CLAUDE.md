...
**Page structure**: Each top-level `*.html` file (`index.html`, `about.html`, `cv.html`, `projects.html`, `blog.html`, `resources.html`, `testimonials.html`, `work-with-me.html`, `contact.html`, `404.html`) is a fully self-contained page, but shares two files across all of them (see below): `shared.css` for common styling and `icons.svg` for icon markup. Anything left in a page's own inline `<style>` block is genuinely page-specific (layout, component styling for that page's unique cards/sections).
